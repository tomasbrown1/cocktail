<?php
$pageTitle = 'Brandy Sour';
$pageDescription = 'How to make a Brandy Sour';
include ('includes/header.html')
?>

				<img src="img/placeholder.jpg" alt="Amaretto Sour" class="photo">
				<h2>Name:</h2>
				<p>Brandy Sour</p>
				<h2>Glassware:</h2>
				<p>Old Fashioned</p>
				<h2>Ingredients:</h2>
				<ul>
					<li>15ml Brandy</li>
					<li>25ml Lemon Juice</li>
					<li>15ml Sugar Syrup</li>
					<li>1 Egg White</li>
					<li>Bitters</li>
					<li>Slices of Lemon</li>
					<li>Cherry</li>
				</ul>
				<h2>Method:</h2>
				<ul>
					<li>Whisk the egg white up in the boston with a bar spoon until it’s foamy and full of air</li> 
					<li>Add the brandy, lemon juice, sugar syrup and bitters and shake vigorously</li> 
					<li>Strain into an Old Fashioned glass full of ice</li>
					<li>garnish with a lemon slice and cherry</li>
				</ul>
				<h2>History:</h2>
				<p>A cocktail from the ‘Sours’ category</p>
				<h2>Variations:</h2>
				<p>
					<a href="whiskeysour.php">Whiskey Sour</a><br />
					<a href="ginsour.php">Gin Sour</a><br />
					<a href="amarettosour.php">Amaretto Sour</a><br />
					<a href="piscosour.php">Pisco Sour</a>
				</p>


<?php
include ('includes/footer.html')
?>